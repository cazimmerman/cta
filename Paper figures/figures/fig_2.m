...
ylabel('Flavour preference (%)'); ylim([0 50]); yticks(0:25:50);
set(gca,'FontSize',12,'LineWidth',1,'TickLength',[0.015, 0],'TickDir','out')
hold off
...
idx_licl = find(cellfun(@(x) isequal('Malaise',x),data.GLMMinput.timepoint));
idx_cgrp = find(cellfun(@(x) isequal('CGRP stim',x),data.GLMMinput.timepoint));
...
total = data.GLMMinput.totalcounts./data.GLMMinput.pbcounts;
idx = sort([idx_nov;idx_fam]);
counts_norm(intersect(idx_licl,idx),:) = data.GLMMinput.counts(intersect(idx_licl,idx),:)./data.GLMMinput.pbcounts(intersect(idx_licl,idx))./mean(total(intersect(idx_licl,idx)));
counts_norm(intersect(idx_cgrp,idx),:) = data.GLMMinput.counts(intersect(idx_cgrp,idx),:)./data.GLMMinput.pbcounts(intersect(idx_cgrp,idx))./mean(total(intersect(idx_cgrp,idx)));
...
axis square
for j = 1:3
    idx = cellfun(@(x) isequal(x,'Novel'),T.Flavour);
    x = T{idx,j+1}*100;
    xx = [NaN mean(x)-std(x)./sqrt(length(x)) mean(x) mean(x)+std(x)./sqrt(length(x)) NaN];
    ...
    idx = cellfun(@(x) isequal(x,'Familiar'),T.Flavour);
    x = T{idx,j+1}*100;
    xx = [NaN mean(x)-std(x)./sqrt(length(x)) mean(x) mean(x)+std(x)./sqrt(length(x)) NaN];
...
xlim([0.5 3.5]); xticks(1:3); xticklabels({'Sst^+','Prkcd^+','Calcrl^+'});
ylabel('Fos^+ CEA cells (%)'); ylim([0 100]); yticks(0:50:100);
legend([b,a],{'Familiar','Novel'})
set(gca,'FontSize',12,'LineWidth',1,'TickLength',[0.015, 0],'TickDir','out')
hold off

idxNov = find(cellfun(@(x) isequal(x,'Novel'),T.Flavour));
idxFam = find(cellfun(@(x) isequal(x,'Familiar'),T.Flavour));
...
xlim([0.5 8.5]); xticks(1:8); xticklabels({'None','Sst^+','Prkcd^+','Calcrl^+','Sst^+/Prkcd^+','Sst^+/Calcrl^+','Prkcd^+/Calcrl^+','All'});
ylabel('Fos^+ CEA cells (%)'); ylim([0 50]); yticks(0:25:50);
legend([b,a],{'Familiar','Novel'})
set(gca,'FontSize',12,'LineWidth',1,'TickLength',[0.015, 0],'TickDir','out')
hold off
