classdef Violin < handle
    ...
    methods
        function obj = Violin(data, pos, varargin)
            ...
            %p = [.29,.11,.34]; w = [1,1,1]; g = [.04,.3,.15];
            %cmap2 = flipud(createcolormap(1000,p,w,g));
            ...
            Y = data;
            X = pos + jitter.*jitterstrength;
            for i = 1:length(X)
                %idx = round((Y(i)+6)*1000/12);
                idx = round((Y(i)+8)*1000/16);
                if idx<1
                    idx = 1;
                elseif idx>length(cmap2)
                        idx = length(cmap2);
                end
                scatter(X(i),Y(i),48,cmap2(idx,:),'o','filled','MarkerEdgeColor','k')
            end

            % plot the mini-boxplot within the violin
            quartiles = quantile(data, [0.25, 0.5, 0.75]);
            %quartiles = [mean(data)-std(data)/sqrt(length(data)) mean(data) mean(data)+std(data)/sqrt(length(data))];
...
%                      [1 1 1]);
            ...
            %plot([pos pos],[quartiles(1) quartiles(3)],'k','LineWidth',1)
...
%obj.MedianPlot = plot(pos+[-.25,.25].*meanDensityWidth,[quartiles(2) quartiles(2)],'k','LineWidth',1);
% 
%             obj.NotchPlots = ...
...
%                          [], [1 1 1], 'filled', 'v');
        ...
        function set.BoxColor(obj, color)
            if ~isempty(obj.BoxPlot)
                obj.BoxPlot.FaceColor = 'k';
                obj.BoxPlot.EdgeColor = 'k';
